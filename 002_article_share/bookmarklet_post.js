(function(){
  ...
  function cleanQt(s){return s.split('\n').map(function(l){return l.replace(/^[ \t　]+/,'');}).filter(function(l){return l.length>0;}).join('\n');}
  var qEl=document.getElementById(Q+'q');
  qEl.value=cleanQt(sel);
  qEl.addEventListener('paste',function(){setTimeout(function(){qEl.value=cleanQt(qEl.value);upd();},0);});
  if(!sel){
    if(navigator.clipboard&&navigator.clipboard.readText){
      navigator.clipboard.readText().then(function(txt){if(txt&&!qEl.value){qEl.value=cleanQt(txt.trim());upd();}}).catch(function(){setTimeout(function(){qEl.focus();},50);});
